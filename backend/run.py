...
def seed_db():
    ...
    dev_pass  = os.environ.get("DEV_USER_PASSWORD")
    if dev_email and dev_pass:
        zak_dev = Zakaznik(jmeno="Dev", prijmeni="User", email=dev_email, telefon=None)
        zak_dev.password = dev_pass
        db.session.add_all([zak1, zak2, zak3, zak_dev])
    else:
        db.session.add_all([zak1, zak2, zak3])

    db.session.flush()

    # přiřazení základních rolí
    zak1.roles.append(user_role)
    zak2.roles.append(user_role)
    zak3.roles.append(user_role)

    # jediná změna: Eva dostává navíc roli staff
    zak2.roles.append(staff_role)

    if dev_email and dev_pass:
        zak_dev.roles.append(admin_role)
