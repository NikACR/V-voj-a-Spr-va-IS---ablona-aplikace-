...
class Rezervace(db.Model):
    """
    Rezervace:
    - datum_cas, pocet_osob, id_zakaznika jsou povinné
    - buď id_stul nebo id_salonek musí být zadáno (CheckConstraint)
    - stav_rezervace, sleva jsou volitelné
    - vztahy: Zakaznik, Stul, Salonek, Notifikace
    """
    ...
    stav_rezervace = db.Column(db.String(20), nullable=True)
...
class PodnikovaAkce(db.Model):
    """
    PodnikovaAkce:
    - nazev, datum, cas, id_salonek povinné; popis volitelný
    - vztah 1:N zpět na Salonek
    """
    ...
    id_salonek = db.Column(
        db.Integer,
        db.ForeignKey("salonek.id_salonek"),
        nullable=False
    )
...
class PolozkaMenu(db.Model):
    """
    PolozkaMenu:
    - nazev, cena povinné; popis volitelný
    - vztahy: objednavky (1:N), alergeny (M:N), plany (1:N)
    """
...
class PolozkaMenuAlergen(db.Model):
    """
    PolozkaMenuAlergen:
    - propojovací tabulka M:N mezi PolozkaMenu a Alergen
    """
